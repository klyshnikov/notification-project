...
[BotCommand("close_wi", "Закрыть WorkItem", Models.CommandAvailabilityScope.Chat | Models.CommandAvailabilityScope.Private)]
internal class CloseWiCommand : IBotCommand
{
    public async Task<BotResponse?> ExecuteAsync(Message message, BotOptions botOptions, CancellationToken cancellationTocken)
    {
        ...
        var result = await httpSender.SendHttpQueryableSimple("close-wi", HttpRequestType.Put, new Dictionary<string, string>
        {
            { "wiId", wiId }
        });
        ...
        {
            return await Task.FromResult<BotResponse?>(new BotResponse("Wi успешно закрыт")).ConfigureAwait(false);
        }
        else
        {
