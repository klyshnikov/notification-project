...
[BotCommand("sub_on_wi_time_remaining", "Подписаться на назначение вам WorkItem", Models.CommandAvailabilityScope.Chat | Models.CommandAvailabilityScope.Private)]
internal class SubOnWiTimeRemainingCommand : IBotCommand
{
    public async Task<BotResponse?> ExecuteAsync(Message message, BotOptions botOptions, CancellationToken cancellationTocken)
    {
        ...
        var result = await httpSender.SendHttpQueryableSimple("sub-on-wi-time-remaining", HttpRequestType.Post, new Dictionary<string, string>
        {
            { "userId", userId }
        });
